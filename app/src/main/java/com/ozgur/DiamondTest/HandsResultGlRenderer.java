...
public class HandsResultGlRenderer implements ResultGlRenderer<HandsResult> {
  ...
  private static final float[] RIGHT_HAND_CONNECTION_COLOR = new float[] {0f, 0f, 0f, 0f};
  private static final float CONNECTION_THICKNESS = 25.0f;
  private static final float[] LEFT_HAND_HOLLOW_CIRCLE_COLOR = new float[] {0f, 1f, 0f, 0f};
  ...
  public void setupRendering() {
    ...
    diamond = BitmapFactory.decodeResource(con.getResources(),
            R.drawable.img);
    //diamond = Bitmap.createBitmap(1030, 1030, icon.getConfig());
    createRgbaTexture(diamond);

  }
  HandsResultGlRenderer(Context con){
      this.con = con;
  }
  @Override
  public void renderResult(HandsResult result, float[] projectionMatrix) {
    ...
    for (int i = 0; i < numHands; ++i) {
      ...
      //drawConnections(result.multiHandLandmarks().get(i).getLandmarkList(), isLeftHand ? LEFT_HAND_CONNECTION_COLOR : RIGHT_HAND_CONNECTION_COLOR);
      for (int j = 0 ;j< result.multiHandLandmarks().get(i).getLandmarkList().size() ;j++) {
        // Draws the landmark.
        if (j == 10) {
          NormalizedLandmark landmark = result.multiHandLandmarks().get(i).getLandmarkList().get(j);
        /*drawCircle(
            landmark.getX(),
            landmark.getY(),
          ...
          drawHollowCircle(
                  landmark.getX(),
                  landmark.getY(), LEFT_HAND_HOLLOW_CIRCLE_COLOR);
        }
      }
    }
  }
  ...
  private void drawHollowCircle(float x, float y, float[] colorArray) {
    ...
    destinationRect.offsetTo((int)x, (int)y);
   ...
   // DataBackBone.getInstance().canvas.drawBitmap(diamond, null, destinationRect, null);

    //GLES20.glActiveTexture(GLES20.GL_TEXTURE_2D);

    //GLES20.glViewport(0, 0, 300, 300);

    //GLES20.glBindTexture(GLES20.GL_TEXTURE_2D,loadTexture(con,R.drawable.img));
    //GLUtils.texImage2D(GLES20.GL_TEXTURE_2D, 0, diamond, 0);
